...
class Student:
    ...
    def average(self):
        Grades = 0.00
        if self.gradesnum > 0 and self.gra:
            for i in range(0, len(self.gra)):
                Grades = Grades + self.gra[i]
            average = Grades / len(self.gra)
            return average
        else:
            return 0.0
...
def train_lora(single_step=False):
    ...
    try:
        ...
        tokenized_data = data.map(tokenize_fn, batched=False, remove_columns=data.column_names)
...
def test():
    ...
    print('AI Chat Test')
    prompt = "I am testing the feature, please introduce yourself."
    ai_chat(prompt)
...
def save():
    if not stu:
        ...
        return
    save_opt = input('Do You Want To Save The Data To a File? (y/n): ')
    try:
        if save_opt.lower() == 'y':
        ...
        elif save_opt.lower() == 'n':
            print('Data Not Saved')
        else:
            print('\n!Invalid Input for Save (should be "y" or "n")!\n')
    except IOError as e:
    ...
    for s in stu:
        if s.name == ename or s.id == ename:
            ...
            try:
                ...
                print(f'\n!Please Enter Number! {ve}\n')
            except Exception as e:
                print(f'!Unexpected Error: {e}!')
            found = True
            break
    if not found:
        print(f"\n!Student Name {ename} Not Found! Please Check The Name And Try Again\n")
...
def handle_ranking():
    ...
    if(ranking == 'hl'):
        ranking_list = quicksorthigh(stu)
        print('Student Ranking by Average Score High to Low')
        for i, s in enumerate(ranking_list, 1):
            print(f"{i}. {s.name} Average: {s.average():.2f}")
    elif(ranking == 'lh'):
        ranking_list = quicksortlow(stu)
        print('Student Ranking by Average Score Low to High')
        for i, s in enumerate(ranking_list, 1):
            print(f"{i}. {s.name} Average: {s.average():.2f}")
    else:
        print('\n!Invalid Input for Ranking (should be "hl" or "lh")!\n')
